...
import com.rahul.orderservice.event.producerEvent.OrderPendingEvent;
...
//    }
    ...
    public Order placeOrder(PlaceOrderRequest request) {
        ...
        for (OrderItemRequest line : request.getItems()) {
            ...
            eventItems.add(new OrderItemPayload(product.getId(), line.getQuantity()));
        }

        Order savedOrder = orderRepository.save(order);

        OrderPendingEvent event = new OrderPendingEvent(savedOrder.getId(), eventItems);
        kafkaTemplate.send(KafkaTopicConfig.ORDER_PENDING, event);
